...
def subtractive_clustering_intensity(img, density1,density2, c):
    # Initialize list of centers with the first pixel in the image
    centers = [(0, 0, img[0][0])]
    # Iterate over every pixel in the image
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            # Check if the difference in intensity between the current pixel and all the centers is greater than the density
            if np.min([abs(img[i][j] - intensity) for _, _, intensity in centers]) > density1:
                if np.min([np.sqrt((i-x)**2 + (j-y)**2) for x, y, _ in centers]) > density2:
                    # If the difference in intensity is greater than the density, add the pixel to the centers
...
centers = subtractive_clustering_intensity(blur,120,120,10)
print("centers")
print(centers)
...
cv2.imshow('New segmented Image', segmented)
cv2.waitKey(0)
